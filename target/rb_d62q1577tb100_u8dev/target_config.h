 ...
 *
 *  非タスクコンテキスト用のスタックは「prc_support.asm」の中でしか操作しない．
 *  スタックポインタの初期値は「$$SP」で取得できる．
 */
#define DEFAULT_ISTKSZ		0x200U
#pragma ASM
	DEFINE DEFAULT_STACKSEG	"0200h"	; /* 「DEFAULT_ISTKSZ」と同じ値に合わせる */
#pragma ENDASM
#define DEFAULT_ISTK		(void *)0xFFFEU	/* コンフィギュレーションを通すためのダミー */
 ...
 */
#define VALID_INTNO(intno)	(1U <= (intno) && (intno) <= 124)
 ...
 *  割込みハンドラの出口で必要なIRC操作
 *
 *  D62Q1577TB100では必要なIRC操作はない．
 */
#pragma INLINE i_end_int
void i_end_int(INTNO intno)
{
}
...
/*
 *  プロセッサ依存モジュール（uX-U16/100用）
 */
#include "xn_u16_100_u8dev/prc_config.h"
